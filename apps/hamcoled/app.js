(function(){
"use strict";
var fs=require("fs"),F="USER/HAMCO_LED.json",leaving=false,torchFn;
...
var S=install();
var CN=["DEFAULT / FIRMWARE","RED","GREEN","BLUE","CYAN","PURPLE","AMBER","WHITE","OFF"];
var EN=["NONE / SOLID","DISCO","SLOW FADE","PULSE","WARNING FLASH","RANDOM SPARK","POLICE LIGHTS","HEARTBEAT","CANDLE FLICKER","LIGHTNING STORM","RADIATION GLITCH","SOS BEACON","PLASMA SHIFT","RAPID STROBE","POWER SURGE","PIP-BOY BOOT"];
function cleanTorch(){try{Pip.removeListener("torch",torchFn)}catch(e){}}
function leave(){if(leaving)return;leaving=true;cleanTorch();try{if(Pip.removeSubmenu)Pip.removeSubmenu()}catch(e){}try{showMainMenu();submenuApps()}catch(e2){try{submenuApps()}catch(e3){try{showMainMenu()}catch(e4){}}}}
function bindTorch(fn){cleanTorch();torchFn=fn;try{Pip.on("torch",torchFn)}catch(e){}}
function tag(on,s){return(on?"> ":"  ")+s}
function effects(){var m={"":{title:"HAMCo LED EFFECTS",selected:S.s.effect}},i;
 for(i=0;i<EN.length;i++)(function(x){m[tag(S.s.effect===x,EN[x])]=function(){S.set(S.s.color,S.s.level,x);effects()}})(i);
 m["< Back"]=main;E.showMenu(m);bindTorch(main)
}
function main(){var m={"":{title:"HAMCo LED CONTROL",selected:S.s.color}},i;
 for(i=0;i<CN.length;i++)(function(x){m[tag(S.s.color===x,CN[x])]=function(){S.set(x,S.s.level,0);main()}})(i);
 m["Brightness"]={value:Math.round(S.s.level*4),min:1,max:4,step:1,format:function(v){return v*25+"%"},onchange:function(v){S.set(S.s.color,v/4,S.s.effect)}};
 m["Effects: "+EN[S.s.effect]]=effects;
 m["Reset Default + Exit"]=function(){S.set(0,1,0);leave()};
 m["Exit / Keep Current"]=function(){save(S.s);leave()};
 E.showMenu(m);bindTorch(leave)
}
main();
}());
